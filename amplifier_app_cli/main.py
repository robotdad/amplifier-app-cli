...
class CommandProcessor:
    ...
    COMMANDS = {
        ...
        "/reload-commands": {"action": "reload_commands", "description": "Reload custom commands from disk"},
    }
            ...
            if hasattr(slash_cmd_tool, "registry") and slash_cmd_tool.registry:
                ...
                if hasattr(registry, "is_loaded") and registry.is_loaded():
                    ...
                    if self.custom_commands:
                        logger.debug(f"Loaded {len(self.custom_commands)} custom commands")
    ...
    def reload_custom_commands(self) -> int:
        ...
        try:
            ...
            if hasattr(slash_cmd_tool, "registry") and slash_cmd_tool.registry:
                ...
                for cmd_name, cmd_data in slash_cmd_tool.registry.get_command_dict().items():
                    key = f"/{cmd_name}"
                    self.custom_commands[key] = {
                        "action": "execute_custom_command",
                        "description": cmd_data.get("description", "Custom command"),
    ...
    def process_input(self, user_input: str) -> tuple[str, dict[str, Any]]:
        ...
        if user_input.startswith("/"):
            ...
            if command in self.custom_commands:
                ...
                return cmd_info["action"], {"args": args, "command": command, "metadata": cmd_info["metadata"]}
    ...
    async def _execute_custom_command(self, data: dict[str, Any]) -> str:
        ...
        try:
            ...
            slash_cmd_tool = tools.get("slash_command")
            if not slash_cmd_tool:
                return f"Error: slash_command tool not loaded"
    ...
    def _format_help(self) -> str:
        ...
        if self.custom_commands:
            ...
            lines.append("")
            lines.append("Tip: Use /reload-commands to reload custom commands from disk")
